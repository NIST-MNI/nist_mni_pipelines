...
if [ ! -e test_lib_1/library.json ];then
# create the library
export ITK_GLOBAL_DEFAULT_NUMBER_OF_THREADS=1

python -m scoop -n 4 $PREFIX/iplScoopFusionGrading.py  \
  --create library_description.json --output test_lib_1 --debug
fi
...
mkdir -p test_cv_slow_1

export ITK_GLOBAL_DEFAULT_NUMBER_OF_THREADS=1
python -m scoop -n 4 -vvv \
  $PREFIX/iplScoopFusionGrading.py \
   --output test_cv_slow_1 \
   --debug  \
   --grade test_lib_1 \
   --cv cv.json \
   --options grade_slow.json 
