...
cat - > library_description_${V}.json <<END
{
  "reference_model": "adni_ec_bbox/model_t1w.mnc",
  "reference_mask":  "adni_ec_bbox/model_mask.mnc",
  ...
  "pre_filters":  {"denoise":true},
  "post_filters": {"label_norm":{"order":1}}
}
END


if [ ! -e test_lib_${V}/library.json ];then
# create the library
export ITK_GLOBAL_DEFAULT_NUMBER_OF_THREADS=1

python -m scoop -n 4 $PREFIX/iplScoopFusionSegmentation.py  \
  --create library_description_${V}.json --output test_lib_${V} --debug
fi
...
export ITK_GLOBAL_DEFAULT_NUMBER_OF_THREADS=1
python -m scoop -n 2 -vvv \
  $PREFIX/iplScoopFusionSegmentation.py \
   --output test_cv_${V} \
   --debug  \
   --segment test_lib_${V} \
   --cv cv_${V}.json \
   --options segment_${V}.json 
