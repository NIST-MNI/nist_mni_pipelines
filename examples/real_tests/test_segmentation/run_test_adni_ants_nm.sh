...
if [ ! -e test_lib_nl_np/library.json ];then
# create the library
export ITK_GLOBAL_DEFAULT_NUMBER_OF_THREADS=1

python -m scoop -n 4 $PREFIX/iplScoopFusionSegmentation.py  \
  --create library_description.json --output test_lib_nl_np --debug
fi
...
mkdir -p test_cv_nl_np

export ITK_GLOBAL_DEFAULT_NUMBER_OF_THREADS=1
python -m scoop -n 4 -vvv \
  $PREFIX/iplScoopFusionSegmentation.py \
   --output test_cv_nl_np \
   --debug  \
   --segment test_lib_nl_np \
   --cv cv.json \
