...
cat - > library_description.json <<END
{
  ...
  "non_linear_register_type": "elastix",
  "non_linear_register_level": 2,
  
  "non_linear_register_options": 
  { 
    "grid_spacing": 4, 
    "pyramid":"8 8 8 4 4 4 2 2 2", 
    "resolutions":3
  },
...
if [ ! -e test_lib_elx3/library.json ];then
# create the library

python -m scoop -n $PARALLEL $PREFIX/iplScoopFusionSegmentation.py  \
  --create library_description.json \
  --output test_lib_elx3 --debug
fi
