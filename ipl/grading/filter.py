...
def filter_sample(input,output,filters,model=None):
    apply_filter(input.scan,output.scan,filters,model=model.scan,input_mask=input.mask,model_mask=model.mask)
    # TODO: parallelalize?
    for (i,j) in enumerate( input.add ):
        apply_filter(input.add[i],output.add[i],filters,model=model.add[i],input_mask=i.mask,model_mask=model.mask)
            ...
            with mincTools() as m:
                ...
                if filters.get('normalize',False) and model is not None:
                    if filters.get('nuyl',False):
                        m.nuyl_normalize(output_scan,model,m.tmp('normalized.mnc'),source_mask=input_mask,target_mask=model_mask)
                    else:
                        m.volume_pol(output_scan,model,    m.tmp('normalized.mnc'),source_mask=input_mask,target_mask=model_mask)
...
def create_unflip_remap(remap,remap_flip):
    if remap is not None and remap_flip is not None:
        ...
        _remap_flip={ int(i[0]):int(i[1]) for i in remap_flip }
        _rr={}
