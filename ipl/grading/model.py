...
def create_fake_mask(in_scan, out_mask, op=None ):
    try:
        with mincTools() as m :
            if op is None :
                m.calc([in_scan], 'A[0]>0.5?1:0', out_mask, labels=True)
            else :
                m.binary_morphology(in_scan, op, out_mask, binarize_threshold=0.5)
