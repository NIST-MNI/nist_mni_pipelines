...
def resample_split_segmentations(input, output,xfm=None, like=None, order=4, invert_transform=False, symmetric=False):
    ...
    if symmetric:
        ...
        for (i,j) in input.seg_f_split.items():
            ...
            results.append(
                resample_file.remote(j,output.seg_f_split[i],xfm=xfm,like=like,order=order,invert_transform=invert_transform
            ))
    ...
    try:
        with mincTools() as m:
            ...
            seg_output_f=output.seg_f
                ...
                #seg_output=model.seg
            
            #if seg_output_f is None:
                #seg_output_f=model.seg
            ...
            m.resample_smooth(sample.scan, output_scan, transform=xfm, like=model.scan, order=resample_order, invert_transform=invert_transform)
            ...
            for (i,j) in enumerate( sample.add ):
                ...
                m.resample_smooth(sample.add[i], output_scan, transform=xfm, like=model.scan, order=resample_order,invert_transform=invert_transform)
            ...
            if symmetric:
                ...
                m.resample_smooth(scan_f, output_scan_f, transform=xfm_f, like=model.scan, order=resample_order,invert_transform=invert_transform)
                ...
                for (i,j) in enumerate( sample.add_f ):
                    ...
                    m.resample_smooth( sample.add_f[i], output_scan_f, transform=xfm_f, like=model.scan, order=resample_order,invert_transform=invert_transform)
