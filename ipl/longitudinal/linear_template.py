...
def generate_skull_mask_dist_map(in_brain_skull, outertable, out_mask, out_dist, width1=4, width2=3, top=None):
    with mincTools() as m:
        ...
        # TODO: replace in_brain_skull with m.tmp("brain_skull_c3.mnc")
        m.calc([m.tmp("brain_skull_c3.mnc"), m.tmp("brain_skull_d.mnc")], 
               "A[1]>0.5&&A[0]<0.5?1:0", outertable, datatype="byte")
        ...
        if top is not None:
            m.calc([m.tmp("brain_skull_dist.mnc"), m.tmp("brain_top.mnc")] ,"A[1]>0.5?abs(A[0]):0",out_dist)
        else:
...
def register_using_skull(in_t1w, in_skull,     in_brain, 
    ...
    if not os.path.exists(out_lin_xfm):
        ...
        with mincTools() as m:
            ...
            generate_skull_mask_dist_map(m.tmp("in_skull.mnc"), m.tmp("in_mask.mnc"),  
                                         m.tmp("in_mask2.mnc"), m.tmp("in_dist.mnc"), 
                                         top=top)
            
            generate_skull_mask_dist_map(in_ref_skull, m.tmp("ref_mask.mnc"),
                                         m.tmp("ref_mask2.mnc"), m.tmp("ref_dist.mnc"), 
                                         top=top)
            ...
            m.xfmconcat([m.tmp("s0.xfm"),m.tmp("s1.xfm")], out_lin_xfm)
