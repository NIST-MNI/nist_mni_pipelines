  ...
  <Plugins>
   ...
   <SEEGAtlas/>
    ...
    xml_modified = default_xml.substitute(
                        replace_main_image_path = os.path.relpath(iter_summary['t1w_nuc'].scan, iter_summary['output_dir']),
                        replace_main_image_name = iter_summary['t1w_tal_noscale'].name,
                        replace_mask_path = os.path.relpath(iter_summary['t1w_masked'].scan, iter_summary['output_dir']),
                        replace_mask_name = iter_summary['t1w_masked'].name,
                    ...
                    if surfaces_parameters.get('skin',False) \
                        ...
                        if surfaces_parameters.get('cortex',False) :
                            with mincTools(verbose=2) as minc:
                                ...
                                #resample the masked brain scan in original patient space
                                minc.command(['mincresample','-invert_transformation',
                                              '-transformation',t1w_tal_noscale_xfm.xfm,
                                              '-like', t1w_nuc.scan,
                                              t1w_tal_noscale_masked.scan, t1w_masked.scan])
